...
const getReplaceString = (replaces) => {
    ...
    for(let i = 0; i < replaces.length / 2; i++) {
        ...
        const fromUpperSing = capitalize(fromSing)
        const fromUpperPlu = capitalize(fromPlu)

        const toUpperSing = capitalize(toSing)
        const toUpperPlu = capitalize(toPlu)
...
const replaceStr = (str, replaces) => {
    let fromStr = str;
    replaces.map((item) => {
        fromStr = fromStr.replace(new RegExp(item.from, "i"), item.to)
    })
